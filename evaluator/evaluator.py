...
class Evaluator:
    ...
    def compute_l1(self):
        self.l1 = self.criterionL1(
            self.gt_images / 2 + 0.5, self.generated_images / 2 + 0.5
        ).item()

    def compute_ssim(self):
        self.ssim = self.criterionSSIM(
            self.gt_images / 2 + 0.5, self.generated_images / 2 + 0.5
        ).item()

    def compute_msssim(self):
        self.msssim = self.criterionMSSSIM(
            self.gt_images / 2 + 0.5, self.generated_images / 2 + 0.5
        ).item()
    ...
    def compute_final_results(
        ...
        for file in files:
            ...
            if file != "final_results.txt":
                ...
                with open(file_path, "r") as f:
                    loss = f.read().split("\n")
                    num_images += int(loss[0])
                    num_correct += int(loss[1])
                    l1 += float(loss[2]) * int(loss[0])
                    ssim += float(loss[3]) * int(loss[0])
                    msssim += float(loss[4]) * int(loss[0])
                    fid += float(loss[5]) * int(loss[0])
        ...
        with open(os.path.join(self.out_root, "final_results.txt"), "w") as f:
            f.writelines(res)
            print(
                "results saved at {}".format(
                    os.path.join(self.out_root, "final_results.txt")
                )
            )
...
class EvaluatorDataset(Dataset):
    def __init__(self, opt):
        data_root = os.path.join(
            opt.results_dir, opt.name, opt.phase + "_{}".format(opt.epoch), "images"
        )
    ...
    def __getitem__(self, idx):
        ...
        gt_images, generated_images = sorted(self.table[key][0]), sorted(
            self.table[key][1]
        )
        gt_images = torch.cat([self.load_image(path) for path in gt_images], 0)
        generated_images = torch.cat(
            [self.load_image(path) for path in generated_images], 0
        )
