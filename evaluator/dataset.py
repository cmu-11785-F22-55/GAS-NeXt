...
class ClassifierDataset(Dataset):
    def __init__(self, mode):
        ...
        classes = sorted(
            list(set(path.split(os.sep)[part] for path in all_image_paths))
        )
        self.text2label = dict(zip(classes, [i for i in range(len(classes))]))
        self.num_classes = len(classes)
        self.label2text = dict(zip([i for i in range(len(classes))], classes))
        self.data = list(
            zip(
                all_image_paths,
    ...
    def get_all_paths(self, roots):
        ...
        for root in roots:
            ...
            for font in fonts:
                ...
                character_paths = [
                    os.path.join(font_path, character) for character in characters
                ]
